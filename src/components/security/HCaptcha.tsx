...
function ensureHCaptchaScript(): Promise<void> {
  ...
  if (!hcaptchaScriptPromise) {
    hcaptchaScriptPromise = new Promise((resolve, reject) => {
      ...
      if (existing) {
        if (existing.dataset.hcaptchaLoader === 'loaded' || existing.getAttribute('data-hcaptcha-loaded') === 'true') {
          ...
          const handleLoad = () => {
            ...
            scriptLoadAttempts = 0; // Reset on success
            console.warn('[hCaptcha] Script loaded successfully');
            waitForHCaptchaReady(resolve, reject);
          };
      ...
      console.warn('[hCaptcha] Loading script...');
      ...
      window[CALLBACK_NAME] = () => {
        ...
        scriptLoadAttempts = 0; // Reset on success
        console.warn('[hCaptcha] Script loaded and callback executed');
        waitForHCaptchaReady(resolve, reject);
      };
...
export default function HCaptcha({ siteKey, onVerify, onExpire, onError, className }: HCaptchaProps) {
  ...
  useEffect(() => {
    ...
    console.warn('[hCaptcha] Initializing widget with site key:', siteKey.substring(0, 8) + '...');
    ...
    ensureHCaptchaScript()
      .then(() => {
        ...
        console.warn('[hCaptcha] Rendering widget...');
        try {
          widgetIdRef.current = window.hcaptcha.render(containerRef.current, {
            sitekey: siteKey,
            callback: (token: string) => {
              console.warn('[hCaptcha] Verification successful');
          ...
          console.warn('[hCaptcha] Widget rendered successfully with ID:', widgetIdRef.current);
    ...
    return () => {
      ...
      if (widgetIdRef.current !== null && window.hcaptcha) {
        try {
          console.warn('[hCaptcha] Cleaning up widget:', widgetIdRef.current);
